;;; $Id: advise.el,v 1.4 1993/01/08 21:58:38 dgg Exp $
...
;;; Patched 1/7/93 to work for nested advise on autoloaded functions -salem
...
(provide 'advise)
(require 'cl)				;definition of unless
...
(defun add-advise-internal (function class list-of-forms)
  ...
  (let* ((old-function-cell (symbol-function function))
	 (autoload-file (and (listp old-function-cell)
			     (and (eq (car old-function-cell) 'autoload)
				  (cadr old-function-cell))))
	 (advised-autoload-p (and (listp old-function-cell)
				  (equal (fourth old-function-cell)
				      ''autoload-advise)
				  ))
	 )
    ...
    (setf (symbol-function function)
	  (cond (advised-autoload-p
		  ;; Take this branch if it's an autoload function that's
		  ;; already been advised
		  (let ((cell (last old-function-cell)))
		    (setf (cdr cell) (list (car cell)))
		    (setf (car cell)
			  (` (add-advise-internal
