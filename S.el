...
(if inferior-S-mode-map
    nil
;; copying doesnt seem to be necessary for v19.  At least,
;; the full-copy-sparse-keymap function no longer exists, and
;; has a comment about it being vor v18
;  (setq inferior-S-mode-map (full-copy-sparse-keymap comint-mode-map))
  (setq inferior-S-mode-map comint-mode-map)
...
to continue it."
  ...
  (setq scroll-step 4)
;gary: merge all ring-sizes together
...
(defun S-quit ()
  ...
  (let ((sprocess (get-process "S")))
	...
	(save-excursion 
	  ...
	  (S-switch-to-S nil)
	  (goto-char (marker-position (process-mark sprocess)))
	  (when (y-or-n-p "S-quit: Save input history before exiting? ")
...
;duplicate binding for gary, new S mode uses this
